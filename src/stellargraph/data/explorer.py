...
class BiasedRandomWalk(RandomWalk):
    ...
    def run(
        ...
        if weighted:
            ...
            # the walk
            def transition_probability(nn): # nn - neighbour in neighbours
                nn, weight = nn
        ...
        walks = []
        for node in nodes:  # iterate over root nodes (over all nodes in the graph)
            for walk_number in range(n):  # generate n walks per root node
                ...
                for _ in range(length - 1):
                    ...
                    # print('choice ',neighbours[choice])
